"""
LOOM Bootstrap — 環境自適應初始化器。

用法：
  python3 -m loom.install.bootstrap          # 交互式檢測+安裝
  python3 -m loom.install.bootstrap --auto    # 全自動（默認值）
  python3 -m loom.install.bootstrap --hermes  # 裝入 Hermes venv
  python3 -m loom.install.bootstrap --venv    # 獨立 venv

設計：
  1. 先檢測環境（Hermes/WSL/GPU/llama-server）
  2. 生成 loom.yaml（自适应注入檢測值）
  3. 創建數據目錄 + 初始化各模塊
...
def generate_loom_yaml(
    ...
    """根據環境檢測結果生成 loom.yaml 內容。"""
    # 數據目錄
    if not data_dir:
        data_dir = str(Path.home() / ".loom" / "data")
    ...
    # 構建 YAML
    lines = []
    lines.append("# LOOM 配置 — 由 loom-bootstrap 自動生成")
    ...
    lines.append("# ── Chroma ──")
    lines.append("chroma:")
    lines.append("  collection: loom_knowledge")
    lines.append("")
    ...
    lines.append("  vectors_path: ~/.loom/worker_vectors.pkl")
    lines.append("  status_cache: ~/.loom/status_cache.pkl")
    lines.append("")
...
def init_chroma(data_dir: str, collection: str = "loom_knowledge"):
    """初始化 ChromaDB（創建集合）。"""
    try:
        import chromadb
...
def init_centroids():
    ...
    try:
        ...
        from loom.knowledge.classify import classify
        vs = VectorStore()
        count = vs.count()
        if count > 0:
...
def deploy_pulse_manager(is_wsl: bool, hermes_available: bool):
    ...
    script_path = Path.home() / ".loom" / "bin" / "pulse-check.py"

    # 寫入 pulse-check 腳本
    script_content = """#!/usr/bin/env python3
\"\"\"LOOM Pulse Check — WSL 脈動探測器。

每次 tick = 檢查所有到期任務。不常駐——由 Hermes cron（每 15min）喚醒。
LOOM 已 pip install -e 安裝，直接 import 即可。
\"\"\"
import sys
from pathlib import Path
...
try:
    ...
    sys.path.insert(0, str(Path.home() / "LOOM" / "src"))
    from loom.finder.heartbeat import Heartbeat
    hb = Heartbeat()
    n = hb.tick()
...
def install_into_hermes(hermes_info: dict, project_root: str) -> bool:
    ...
    try:
        ...
        if r.returncode == 0:
            print(f"  ✓ LOOM installed into Hermes venv: {venv_python}")
            # 驗證導入
            verify = subprocess.run(
                [venv_python, "-c", "import loom; print(loom.__version__)"],
                capture_output=True, text=True, timeout=10,
            )
            if verify.returncode == 0:
                ver = verify.stdout.strip()
                print(f"  ✓ Verification: import loom -> v{ver}")
            return True
        else:
...
def _install_loom_tool_symlink(hermes_available: bool):
    ...
    hermes_home = os.getenv("HERMES_HOME", str(Path.home() / ".hermes"))
    tool_dst = Path(hermes_home) / "hermes-agent" / "tools" / "loom_tool.py"
...
def _install_yicenet_soft(target_python: str = ""):
    ...
    try:
        ...
        cmd += ["--skip-hermes"]  # Hermes 工具鏈接由 LOOM bootstrap 管理
...
def bootstrap(auto: bool = False, hermes: bool = False,
    ...
    config_content = generate_loom_yaml(
        hermes_info, is_wsl, gpu, llama, providers, data_dir,
    )

    config_path = Path.home() / ".loom" / "loom.yaml"
    ...
    resolved_data = data_dir or str(Path.home() / ".loom" / "data")
    ...
    project_root = os.path.dirname(project_root)  # loom/ -> src/ -> project root
    ...
    if not skip_install:
        ...
        # LOOM Hermes tool symlink (optional)
        _install_loom_tool_symlink(hermes_info.get("available", False))
        print()
    else:
    ...
    # ── 完成 ──
    print("╔══════════════════════════════════════════╗")
    print("║  LOOM Bootstrap 完成！                  ║")
    print("╚══════════════════════════════════════════╝")
    print()
    print("  驗證安裝:")
    print(f"    python3 -c \"import loom; print(loom.__version__)\"")
    print(f"    loom-heartbeat")
    print()
    print("  配置位置:")
    print(f"    ~/.loom/loom.yaml")
    print()
    print("  數據目錄:")
...
def main():
    parser = argparse.ArgumentParser(
        description="LOOM Bootstrap — 環境自適應初始化",
    )
