"""
classify.py — Pure embedding-based classification for LOOM.
...
def _classify_level(text: str) -> str:
    ...
    try:
        from loom.knowledge.classify.sub_registry import LevelRegistry as LR
        reg = LR.instance()
...
def _classify_type(text: str) -> str:
    ...
    try:
        from loom.knowledge.classify.sub_registry import TypeRegistry as TR
        reg = TR.instance()
        best, score, _ = reg.classify_text(text)
        if best and score > 0.3:
            return best.name
