...
from loom.finder.status_cache import StatusCache, StatusEntry, compute_freshness
...
class Heartbeat:
    ...
    def _probe_cloud(self, provider: str) -> dict:
        ...
        端點從 LOOM config 的 providers 獲取。
        """
        start = time.time()
...
def ensure_running():
    ...
    from loom.config import get_config
    cfg = get_config()
    cron_name = cfg.heartbeat_cron_name  # "loom-heartbeat"
    ...
    try:
        ...
        subprocess.run(
            ["hermes", "cron", "create",
             "--name", cron_name,
             "--schedule", "*/2 * * * *",
             "--script", "python3 -m loom.finder.heartbeat",
             ],
            capture_output=True, text=True, timeout=10,
        )
