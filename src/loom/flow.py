...
def flow_enabled() -> bool:
    ...
    if _ENABLED is not None:
        return _ENABLED
    # When Shuttle is active, suppress per-step flow rendering
    if os.getenv("LOOM_SHUTTLE", "").lower() in ("1", "true", "yes", "on"):
        return False
    env = os.getenv("LOOM_FLOW")
    if env is not None:
