"""LOOM CLI — `python -m loom`.

用法:
    python -m loom query <question>          查詢知識庫
    python -m loom ingest <file.md>          攝入文檔
    python -m loom stats                     系統統計
    python -m loom kpak pack <domain>        導出知識包
    python -m loom kpak unpack <file.kpak>   導入知識包
    python -m loom kpak list                 列出知識包
"""
...
def cmd_kpak(args):
    if args.kpak_action == "pack":
        from loom.kpak.pack import pack_domain
        pack_domain(args.domain, args.output)
    elif args.kpak_action == "unpack":
        from loom.kpak.unpack import unpack_file
...
def main():
    parser = argparse.ArgumentParser(prog="loom", description="LOOM 知識引擎")
