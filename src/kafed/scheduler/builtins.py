"""KAFED Scheduler — 內建任務。

註冊 KAFED 的定時維護任務到 TaskRegistry。
...
from kafed.scheduler.registry import Task, TaskResult, SimpleTask, registry

logger = logging.getLogger("kafed.scheduler.builtins")
...
def _centroid_rebuild() -> TaskResult:
    """重建 domain centroid。"""
    try:
        from kafed.knowledge.classify.classify import rebuild_centroids
        rebuild_centroids()
...
def _knowledge_audit() -> TaskResult:
    """離線知識庫稽核。"""
    try:
        from kafed.analyzer.knowledge_audit import KbAuditor
        auditor = KbAuditor()
...
def _flywheel_daily() -> TaskResult:
    ...
    try:
        from kafed.knowledge.flywheel_events import check_events
        events = check_events()
