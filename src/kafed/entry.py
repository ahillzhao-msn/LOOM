...
方法與 Pipeline 步驟映射：
  ...
  find_partners(briefs)      → 決（Agent 直調 Finder，不經中間層）
  execute(plan)              → 編（DAG 執行）
  solidify(insight, ...)     → 固（知識固化）
  backlog_*()                → 跨 session 待辦
  session_*()                → session 生命周期

注意：決 step 不再有 plan() 函數。Agent（Director）直接調用
get_router().find_partners(briefs=[...]) 完成模型匹配，
保留最終決定權。分解決策參考見 references/pipeline/decompose-guide.md。

數據結構：SubTask、TaskPlan、TaskResult 使用 director/planner.py 的規範定義，
避免 entry 層與 director 層的重複 DTO。
"""
...
# ── Pipeline 數據結構（引用 canonical 定義）──
from kafed.director.planner import SubTask, TaskPlan
...
def execute(plan: TaskPlan,
            feedback_callback: Optional[Any] = None) -> list[TaskResult]:
    """E 橋接：TaskPlan → ExecutorEngine DAG 執行 → TaskResult[]。"""
    from kafed.executor.engine import ExecutorEngine, default_feedback_callback
