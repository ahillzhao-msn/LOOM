...
class Explorer:
    ...
    class PricingTable:
        ...
        定價緩存文件: ~/.kafed/pricing_cache.json
        ...
        def default_path() -> Path:
            """定價緩存文件的默認路徑。"""
            return Path.home() / ".kafed" / "pricing_cache.json"
    ...
    def _get_pricing_cache_path() -> Path:
        """定價緩存文件路徑（可被 KAFED_PRICING_CACHE 環境變量覆蓋）。"""
        env = os.getenv("KAFED_PRICING_CACHE")
        if env:
