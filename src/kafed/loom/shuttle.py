...
取代 KAFED flow.py 的 FlowVisualizer——不再局限於每輪流程鏈，
而是跨層級、跨輪次的一體化可視化。
"""
...
def shuttle_enabled() -> bool:
    env = os.getenv("KAFED_SHUTTLE", os.getenv("KAFED_FLOW", "1"))
...
class Shuttle:
    ...
    def flow_chain(steps: list[str], end: str = "") -> str:
        ...
        輸入: ["D問", "D卦(YiCeNet)", "D召(KAFED)", "D評(EVAL)"]
        輸出: D問 -> D卦(YiCeNet) -> D召(KAFED) -> D評(EVAL)
        """
        line = " -> ".join(steps)
        if end:
            line += f" -> {end}"
